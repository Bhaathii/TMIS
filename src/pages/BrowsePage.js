...
import { getAllTMISHeaders, searchTMISByTapeCode, deleteTMISHeader, logActivity } from '../utils/firebaseUtils';
...
export const BrowsePage = () => {
    ...
    const handleDelete = async (header) => {
        ...
        try {
            ...
            logActivity(currentUser?.uid || '', currentUser?.email || '', 'delete', 'TMIS_Header', `Deleted record: ${header.TapeCode}`, header.id).catch(err => console.error('Failed to log activity:', err));
            // Reload headers
            loadHeaders();
        }
        catch (error) {
