...
import { getTMISHeader, updateTMISHeader, addTMISHeader, logActivity, getLayersByHeaderId, getProcessesByHeaderId, getSlittingsByHeaderId, addTMISLayer, updateTMISLayer, addTMISProcess, updateTMISProcess, addTMISSlitting, updateTMISSlitting, } from '../utils/firebaseUtils';
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (isEditMode && headerId) {
                ...
                logActivity(currentUser?.uid || '', currentUser?.email || '', 'edit', 'TMIS_Header', `Edited record: ${header.TapeCode}`, headerId).catch(err => console.error('Failed to log activity:', err));
                // Handle layers
                ...
                // Log activity (non-blocking - don't throw errors)
                logActivity(currentUser?.uid || '', currentUser?.email || '', 'create', 'TMIS_Header', `Created new record: ${header.TapeCode}`, headerId).catch(err => console.error('Failed to log activity:', err));
                // Add child records
                for (const layer of layers) {
                    const { id: _, ...layerData } = layer;
                    await addTMISLayer({
