...
export const AuthProvider = ({ children }) => {
    ...
    useEffect(() => {
        if (prevUserRef.current === null && currentUser !== null) {
            recordLogin(currentUser.uid, currentUser.email).catch(err => console.error('Failed to record login:', err));
            logActivity(currentUser.uid, currentUser.email, 'login', 'AUTH', `User logged in`).catch(err => console.error('Failed to log activity:', err));
        }
